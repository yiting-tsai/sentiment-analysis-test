...
4. create a virtual environment `virtualenv myEnv` and activate `source myEnv/bin/activate`
5. install necessary python packages `pip install -r requirements`
6. run on terminal the actual inference script `python xlmr-inference.py` (apprx. 10 mins to finish if on CPU)
